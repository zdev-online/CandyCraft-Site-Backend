...
export class ShopService {
    ...
    async buy(id: number, @User() user: UserFromRequest) {
        ...
        if (!product) {
            throw new BadRequestException({ message: 'Товар с таким ID - не найден' });
        }
        ...
        try {
            ...
            throw new InternalServerErrorException({ message: 'Что-то пошло не так' });
        }
    }

    async createDonate(dto: CreateDonateDto) {
    ...
    async createItemsForCase(dto: CreateItemsDto) {
        return await this.itemsEntity.create(dto);
    }
    ...
    async deleteProductById(id: number) {
        ...
        product.type == 'donate' && await this.donateEntity.destroy({ where: { id: product.product_id } });
        product.type == 'case' && await this.caseEntity.destroy({ where: { id: product.id } });

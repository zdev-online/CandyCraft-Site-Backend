...
export class AuthController {
  ...
  @ApiOperation({ description: 'Выход из системы', summary: 'Выход из системы' })
  ...
  @UseGuards(AuthGuard)
  @ApiBearerAuth('JWT_AUTH')
  @ApiOperation({ description: 'Обновление Access и Refresh токенов', summary: 'Обновление Access и Refresh токенов' })
  ...
  @UseGuards(AuthGuard)
  @ApiBearerAuth('JWT_AUTH')
  @ApiOperation({ description: 'Подтверждение E-Mail', summary: 'Подтверждение E-Mail' })
  @ApiOkResponse({ type: ConfirmEmailResponseDto })
  @Post('/confirm/email')
  async confirmEmail(@Body('token') token: string, @Res({ passthrough: true }) res: Response): Promise<ConfirmEmailResponseDto> {
  ...
  @ApiOkResponse({ type: MessageResponseDto })
  @Get('/restore')
  async startRestorePassword(@Query('email') email: string): Promise<MessageResponseDto> {

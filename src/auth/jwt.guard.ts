    ...
    async canActivate(ctx: ExecutionContext): Promise<boolean> {
        return new Promise(async (resolve, reject): Promise<boolean> => {
            try {
                ...
                const isForConfirmed = this.reflector.get<boolean>('for_confirmed_email', ctx.getHandler());
                const isForAdmin = this.reflector.get<boolean>('for_admin', ctx.getHandler());
                if(isForConfirmed && !data.confirmed){
                    throw new UnauthorizedException({ message: 'Только для пользователей с подтвержденным E-Mail' });
                }

...
export class PaymentService {
  ...
  async topcraftCallback(data: TopcraftCallbackDto) {
    const t = await this.connection.transaction();
    try {
      let signature = createHash('sha1')
        .update(`${data.username}${data.timestamp}${process.env.TOP_CRAFT_KEY}`)
        .digest('hex');
      console.log(data);
      ...
      throw new InternalServerErrorException(
        'Не удалось зачислить бонусные баллы',
      );
    }
  }
}

...
export class TokensService {
    ...
    private JWT_REFRESH_EXPIRES_NUMBER: number = 1000 * 60 * 60 * 24 * parseInt(process.env.JWT_REFRESH_EXPIRES.replace('d', ''));
    constructor(
    ...
    private async generateAccessToken(payload: TokensPayloadDto): Promise<string> {
        return await this.jwtService.signAsync(payload, {
            expiresIn: this.JWT_ACCESS_EXPIRES,
            secret: this.JWT_ACCESS_SECRET
        });
    }
    private async generateRefreshToken(payload: TokensPayloadDto): Promise<string> {
        return await this.jwtService.signAsync(payload, {
            expiresIn: this.JWT_REFRESH_EXPIRES,
            secret: this.JWT_REFRESH_SECRET
        });
    }
    async createOrRefreshToken(refresh_token: string, userId: number): Promise<Tokens> {
    ...
    private async deleteExpiredTokens(){
        let number = await this.tokensEntity.destroy({
            where: {
                expiresIn: {
                    [Op.lte]: new Date()
                }
            }
        }).catch(e => console.error(`Не удалось очистить просроченные токены: ${e.message}`));

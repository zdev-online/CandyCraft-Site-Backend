...
export class FilesService {
  ...
  async save(files: Express.Multer.File[]) {
    ...
    let results = (await Promise.allSettled(promises)).map(x => x.status == 'fulfilled' ? { ...x, value: x.value } : { ...x, reason: x.reason });
    if(results.some(x => x.status == 'rejected')){
      files.forEach(async x => await this.deleteFromDisk(x.filename));
      throw new InternalServerErrorException({ message: 'Не удалось загрузить файлы', desc: 'Подробную ошибку ищите в логах сервера' });
    }
  ...
  async deleteFromDisk(filename: string) {
    try {
      ...
      throw new InternalServerErrorException({ message: 'Не удалось удалить файл', desc: "Посмотрите логи" });
    }
  }
